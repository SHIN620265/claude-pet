...
- 有对话在跑时,旁边叠出**状态卡**(最多 3 张),每张显示:
  - **标题** = 你在那个对话里说的第一句话。
  - **状态行** = (模型名 ·)当前状态 + 你最近一句输入,带颜色和图标(模型名指该会话**上一条回复**所用模型,`/model` 切换后随下一条回复自动更新):
...
- When conversations are running, **status cards** stack next to it (up to 3). Each card shows:
  ...
  - **Status line** = (model ·) current state + your latest message, with color and icon (the model badge shows what produced the session's **last reply**; after `/model` it catches up with the next reply):
...
- 会話が実行中だと、横に**状態カード**が重なって表示(最大3枚)。各カード:
  - **タイトル** = その会話で最初に入力した文。
  - **状態行** = (モデル名 ·)現在の状態 + 最新の入力(色とアイコン付き。モデル名はその会話の**直前の返信**に使われたモデルで、`/model` 切替後は次の返信で自動更新):
