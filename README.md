...
<p align="center"><img src="docs/demo-jump-v120-en.gif" width="320" alt="Real screen recording: hovering a session card shows a coral ring, and clicking it brings that session's window back to the foreground"></p>
...
3. **跳过去**:**单击卡片 = 把那个会话所在的窗口拉到前台**(悬停出珊瑚色光环;定位不到时卡片会左右摇头,绝不跳错窗口)。
...
### 已知限制
- **点卡片默认跳到窗口;VS Code 内可精确到终端标签**:单击卡片行会把该会话的宿主窗口(Windows Terminal / VS Code / PowerShell)拉到前台,最小化了会先还原。VS Code 用户可另装配套扩展([`vscode-ext/claude-pet-jump`](vscode-ext/claude-pet-jump/),可选),点卡后在窗口内直接聚焦该会话的终端标签;不装则保持窗口级。WT 的多个 tab 暂无可靠的外部 API,落进窗口后的最后一步还需自己点(计划中)。定位不到时(如会话已关闭)卡片会摇头示意,不会瞎跳。
...
3. **Jump**: **single-click a card = bring that session's window to the foreground** (a coral ring appears on hover; the card shakes its head when the window can't be located — it never jumps to the wrong one).
...
3. **ジャンプ**:**カードをクリック = そのセッションのウィンドウを前面に**(ホバーでコーラルのリングが表示。特定できないときはカードが首を横に振り、間違ったウィンドウには決して移動しません)。
...
### 既知の制限
- **カードクリックはウィンドウまで移動(VS Code 内は連携拡張でタブ単位まで)**:クリックでそのセッションのホストウィンドウ(Windows Terminal / VS Code / PowerShell)が前面に出ます(最小化時は復元)。VS Code では連携拡張([`vscode-ext/claude-pet-jump`](vscode-ext/claude-pet-jump/)、任意)を入れると、同じクリックでそのセッションのターミナルタブまでフォーカスします(未導入ならウィンドウ単位のまま)。WT のタブを外部から確実に指定する API はまだないため、ウィンドウ内の最後の一歩はご自身で(対応予定)。特定できない場合(セッション終了後など)はカードが首を振って知らせ、当て推量では移動しません。
