...
**卸载:** `/plugin uninstall claude-pet@shin620265`
...
- **卡片名与终端标签名不自动同步**:卡片标题来自你的第一句话(或手动改名),与终端标签名各自独立。
...
**Uninstall:** `/plugin uninstall claude-pet@shin620265`
...
- **Card name and terminal tab name don't auto-sync**: the card title (your first prompt, or a manual rename) is independent of the tab name.
...
**アンインストール:** `/plugin uninstall claude-pet@shin620265`
...
- **カード名とタブ名は自動同期しません**:カードのタイトル(最初の入力/手動名)はタブ名とは独立。
