...
<p align="center"><img src="docs/demo-live.gif" width="320" alt="Real screen recording: the status card flips to needs-your-input the moment Claude asks, then back to thinking once you approve"></p>
<p align="center"><sub>↑ real screen recording — not a mockup · 中文界面可右键即时切换</sub></p>
...
### 这是什么?
一只待在你 Windows 桌面上的小宠物。它盯着你正在跑的 Claude Code 对话,用**卡片**和**轻提示音**告诉你:Claude 是在**思考**、在**等你做选择**、还是**已经做完了**——这样你不用一直盯着命令行窗口。需要你确认权限时,约 **1 秒**内就会提醒——不必等 Claude Code 自带通知的 6 秒延迟。它也很轻:整个插件**不到 200KB**,没有 Electron、没有捆绑运行时,只是几个 PowerShell 脚本驱动着 Windows 自带的界面组件。
...
3. **跳过去**:**单击卡片行的文字 = 把那个会话所在的窗口拉到前台**(手型光标=可跳;定位不到时卡片会左右摇头,绝不跳错窗口)。
4. **操作**:拖动=移动(记忆位置);双击=收起/展开卡片;每行 **✎**=改标题、**×**=关掉这张卡(有新动静会自己回来);**右键**=菜单(关闭宠物/回默认位置/提示音/语言)。
...
### 已知限制
- **点卡片跳到的是窗口,不是具体标签页**:单击卡片行会把该会话的宿主窗口(Windows Terminal / VS Code / PowerShell)拉到前台,最小化了会先还原;但 WT 的多个 tab、VS Code 内部的多个终端面板没有可靠的外部 API,落进窗口后的最后一步还需自己点。定位不到时(如会话已关闭)卡片会摇头示意,不会瞎跳。
...
### What is this?
A little mascot that lives on your Windows desktop. It watches your running Claude Code conversations and tells you — with **cards** and a **soft chime** — whether Claude is **thinking**, **waiting for your choice**, or **done**. So you don't have to keep staring at the terminal. When Claude needs your permission, you're alerted within about **1 second** — no waiting for Claude Code's built-in 6-second notification delay. It's light, too: the whole plugin is **under 200KB** — no Electron, no bundled runtime, just a few PowerShell scripts driving Windows' built-in UI stack.
...
3. **Jump**: **single-click a card row's text = bring that session's window to the foreground** (hand cursor = jumpable; the card shakes its head when the window can't be located — it never jumps to the wrong one).
4. **Actions**: drag = move (remembered); double-click = collapse/expand cards; per row **✎** = rename, **×** = dismiss (comes back on new activity); **right-click** = menu (close pet / reset position / sound / language).
...
### Known limitations
- **Clicking a card jumps to the window, not the exact tab**: a click brings that session's host window (Windows Terminal / VS Code / PowerShell) to the foreground, restoring it if minimized — but there's no reliable external API to focus a specific WT tab or VS Code terminal pane, so the last hop inside the window is yours. When the window can't be located (e.g. the session is gone), the card shakes its head instead of guessing.
...
### これは何?
Windows のデスクトップに住む小さなマスコットです。実行中の Claude Code の会話を見守り、**カード**と**やさしい通知音**で、Claude が今**考え中**か、**あなたの選択待ち**か、**完了した**かを知らせます。ターミナルをずっと見ていなくても進捗がわかります。権限の確認が必要なときは約 **1 秒**で通知——Claude Code 内蔵通知の 6 秒遅延を待ちません。とても軽量です:プラグイン全体で **200KB 未満**——Electron もバンドルランタイムもなく、数本の PowerShell スクリプトが Windows 標準の UI を動かしているだけです。
...
3. **ジャンプ**:**カード行のテキストをクリック = そのセッションのウィンドウを前面に**(手のカーソル=移動可。特定できないときはカードが首を横に振り、間違ったウィンドウには決して移動しません)。
4. **操作**:ドラッグ=移動(記憶);ダブルクリック=カード折りたたみ/展開;各行 **✎**=名前変更、**×**=カードを閉じる(新しい動きで再表示);**右クリック**=メニュー(ペットを閉じる/位置をリセット/通知音/言語)。
...
### 既知の制限
- **カードクリックで移動できるのはウィンドウまで(タブ単位ではありません)**:クリックでそのセッションのホストウィンドウ(Windows Terminal / VS Code / PowerShell)が前面に出ます(最小化時は復元)。ただし WT のタブや VS Code 内のターミナルパネルを外部から確実に指定する API はないため、ウィンドウ内の最後の一歩はご自身で。特定できない場合(セッション終了後など)はカードが首を振って知らせ、当て推量では移動しません。
