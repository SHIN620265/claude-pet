...
- **批准后卡片可能"迟迟不变"**:你批准权限后,如果那条命令本身要跑很久(构建、测试等),卡片会在整个执行期间停留在"需要你确认"——Claude Code 在"已批准"和"命令跑完"之间不产生任何事件,宠物无从得知你已点过批准。命令结束后自动恢复,无需处理。
...
- **The card may linger on "needs your input" after you approve**: if the approved command itself runs long (builds, test suites), the card keeps that state for the whole run — Claude Code emits no event between "you approved" and "the command finished", so the pet can't know you already clicked. It recovers on its own once the command completes.
...
- **承認後もカードが「確認待ち」のまま残ることがあります**:承認したコマンド自体が長く走る場合(ビルドやテストなど)、実行中はカードがその状態のまま残ります——Claude Code は「承認した」と「コマンドが終わった」の間にイベントを発行しないため、ペットには承認済みだと知る術がありません。コマンド終了後に自動で戻ります。
